...
public class SudokuDLX {
	...
	private static boolean validateSudoku(int[][] grid){
		if(grid.length != 9 && grid.length != 15) return false; // only 9 or 15 for now
		for(int i = 0; i < grid.length; i++){
			if(grid[i].length != grid.length) return false;
			for(int j = 0; j < grid[i].length; j++){
				if(! (i >= 0 && i <= grid.length)) return false; // 0 means not filled in
			}
		}
			...
			for(int j = 0; j < N; j += N/3){
				for(int d1 = 0; d1 < N/3; d1++){
					for(int d2 = 0; d2 < N/3; d2++){
						if(grid[i+d1][j+d2] == 0) continue;
						if(b[grid[i+d1][j+d2]]) return false;
